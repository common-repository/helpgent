...
use HelpGent\App\BackgroundProcessors\MediaTransfer\BackgroundProcessor;

class MediaTransferProcessorProvider implements Provider {
    ...
    public function init_background_processor() {
        ...
        $GLOBALS['helpgent_media_transfer_processor'] = $background_processor;
    }
    ...
    public function run_background_processor() {
        ...
        if ( $helpgent_media_transfer_processor::is_running() ) {
            return;
        }

        $executed_media_transfer_processor = get_transient( 'helpgent_executed_media_transfer_processor' );

        if ( $executed_media_transfer_processor ) {
            return;
        }

        if ( ! ( $helpgent_media_transfer_processor instanceof BackgroundProcessor ) ) {
            return;
        }

        try {
            $helpgent_media_transfer_processor->run();

            $expiration = apply_filters( 'helpgent_media_transfer_processor_execution_expiration', MINUTE_IN_SECONDS * 30 );
            set_transient( 'helpgent_executed_media_transfer_processor', true, $expiration );
