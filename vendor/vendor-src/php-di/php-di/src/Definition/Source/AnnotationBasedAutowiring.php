...
use HelpGent\DI\Annotation\Inject;
use HelpGent\DI\Annotation\Injectable;
use HelpGent\DI\Definition\Exception\InvalidAnnotation;
use HelpGent\DI\Definition\ObjectDefinition;
use HelpGent\DI\Definition\ObjectDefinition\MethodInjection;
use HelpGent\DI\Definition\ObjectDefinition\PropertyInjection;
use HelpGent\DI\Definition\Reference;
use HelpGent\Doctrine\Common\Annotations\AnnotationRegistry;
use HelpGent\Doctrine\Common\Annotations\Reader;
use HelpGent\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use HelpGent\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'HelpGent\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'HelpGent\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('HelpGent\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'HelpGent\\DI\\Annotation\\Injectable');
